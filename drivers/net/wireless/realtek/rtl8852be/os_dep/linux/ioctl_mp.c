			 ...
			 struct iw_point *wrqu, char *extra)
{
	...
	if (wrqu->length == 2) {
		...
		tarpowerdbm = mpt_get_tx_power_finalabs_val(padapter, rfpath);
			if (tarpowerdbm > 0) {
				pextra += sprintf(pextra, "\t\t dBm:%.2d", tarpowerdbm / TX_POWER_BASE);
		...
		pextra += sprintf(pextra, "\n\t\t\tpatha dBm=%.2d", tarpowerdbm / TX_POWER_BASE);
		if (rf_type > RF_1T2R) {
			tarpowerdbm = mpt_get_tx_power_finalabs_val(padapter, 1);
			pextra += sprintf(pextra, ",pathb dBm=%.2d", tarpowerdbm / TX_POWER_BASE);
		}
		if (rf_type > RF_2T4R) {
			tarpowerdbm = mpt_get_tx_power_finalabs_val(padapter, 2);
			pextra += sprintf(pextra, ",pathc dBm=%.2d", tarpowerdbm / TX_POWER_BASE);
		}
		if (rf_type > RF_3T4R) {
			tarpowerdbm = mpt_get_tx_power_finalabs_val(padapter, 3);
			pextra += sprintf(pextra, ",pathd dBm=%.2d", tarpowerdbm / TX_POWER_BASE);
		}
		padapter->mppriv.txpowerdbm = tarpowerdbm;
		  ...
		  struct iw_point *wrqu, char *extra)
{
	...
	if ( pwr_dbm > 0) {
		...
		pextra += sprintf(pextra, "read pwr dbm:%.2d", pwr_dbm / TX_POWER_BASE);
	}
	wrqu->length = strlen(extra);
	return 0;
}
